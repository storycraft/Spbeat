	...
	protected void applyTransform(Batch batch) {
		...
		for (IDrawEffect drawEffect : new ArrayList<>(currentEffectMap.keySet())) {
			if (drawEffect.isEnded(now)) {
				...
				drawEffect.applyAt(now, clone);
			}
		}

		batch.setTransformMatrix(clone.toMatrix4());
	}
	...
	public class DrawableEffectResult implements IEffectResult {
		...
		public void expire() {
			addEffectQueue(new DrawEffect(System.currentTimeMillis(), 0) {
				@Override
				public void applyAt(long currentTime, TransformData transformData) {
		...
		public DrawableEffectResult wait(int time) {
			...
			addEffectQueue(new DrawEffect(time) {
				@Override
				public void applyAt(long currentTime, TransformData graphics) {
		...
		public DrawableEffectResult scaleTo(float scaleX, float scaleY, EasingType type, int duration) {
			...
			addEffectQueue(new DrawEffect(duration) {
				...
				public void applyAt(long currentTime, TransformData transformData) {
					...
					transformData.setScaleX(originalScaleX + (scaleX - originalScaleX) * progress);
					transformData.setScaleY(originalScaleY + (scaleY - originalScaleY) * progress);
				}
		...
		public DrawableEffectResult moveToRelative(float x, float y, EasingType type, int duration) {
			...
			float transformX = getTransformData().getTranslateX();
			float transformY = getTransformData().getTranslateY();
			...
			addEffectQueue(new DrawEffect(duration) {
				...
				public void applyAt(long currentTime, TransformData transformData) {
					...
					transformData.setTranslateX(transformX + -x * (1 - progress));
					transformData.setTranslateY(transformY + -y * (1 - progress));
				}
		...
		public DrawableEffectResult rotateTo(float rotation, EasingType type, int duration) {
			...
			float transformRotation = getTransformData().getRotation();
			...
			addEffectQueue(new DrawEffect(duration) {
				...
				public void applyAt(long currentTime, TransformData transformData) {
					...
					transformData.setRotation(transformRotation + (getRotation() - getStartValue()) * progress - getRotation());
				}
